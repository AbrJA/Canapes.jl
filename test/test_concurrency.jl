...
@testset "Concurrency" begin
    ...
    # GlobalVectors (and PairwiseRanking) is Hogwild lock-free single-pass SGD, so it is excluded
    # from the equality check and asserted for validity only.
    det_model = () -> [
        WeightedMF(rank=8, max_iter=5, verbose=false),
        CachedALS(rank=8, max_iter=5, verbose=false),
        ElementwiseALS(rank=8, max_iter=5, verbose=false),
        LogisticMF(rank=8, max_iter=3, verbose=false),
        ShallowAutoencoder(λ=200.0, verbose=false),
        SparseLinearModel(max_iter=10, verbose=false),
        SparseLinearADMM(max_iter=10, verbose=false),
    ...
    # PairwiseRanking and GlobalVectors use Hogwild! lock-free SGD: concurrent writes to shared
    ...
    @testset "hogwild concurrent training" begin
        models = [PairwiseRanking(rank=8, max_iter=3, verbose=false) for _ in 1:2]
        ...
        glove = [GlobalVectors(rank=8, max_iter=3, verbose=false) for _ in 1:2]
    ...
    @testset "concurrent reads on a shared fitted model" begin
        model = WeightedMF(rank=8, max_iter=5, verbose=false)
        ...
        bpr = PairwiseRanking(rank=8, max_iter=3, verbose=false)
    ...
    @testset "multithreaded subprocess smoke test" begin
        ...
        for m in (WeightedMF(rank=4, max_iter=3, verbose=false), ShallowAutoencoder(λ=100.0, verbose=false))
