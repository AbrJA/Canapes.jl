# test/test_factorizationmachine.jl — Factorization Machine tests

@testset "XOR: rank-2 FactorizationMachine learns interaction" begin
...
@testset "Gaussian family" begin
    ...
    fm = FactorizationMachine(rank=5, family=Gaussian(), lr_w=0.01, max_iter=50, verbose=false)
...
@testset "MSE decreases with iterations (Gaussian)" begin
    ...
    m5 = FactorizationMachine(rank=5, family=Gaussian(), lr_w=0.01, max_iter=5, verbose=false)
    m50 = FactorizationMachine(rank=5, family=Gaussian(), lr_w=0.01, max_iter=50, verbose=false)
...
@testset "Regularization reduces overfitting" begin
    ...
    fm_noreg = FactorizationMachine(rank=5, family=Gaussian(), lr_w=0.01,
                                     λ_w=0.0, λ_v=0.0, max_iter=100, verbose=false)
    fm_reg = FactorizationMachine(rank=5, family=Gaussian(), lr_w=0.01,
...
@testset "Convergence tolerance" begin
    ...
    fm = FactorizationMachine(rank=5, family=Gaussian(), lr_w=0.01,
                               tol=0.001, max_iter=200, verbose=false)
    fit!(fm, Xg, yg; rng=MersenneTwister(1))
