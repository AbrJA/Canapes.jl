...
@testset "interactions_to_sparse with DataFrame" begin
    ...
    @test interactions_to_sparse(nt; user_col=:user, item_col=:item, value_col=:value) == X
end
...
@testset "invalid input errors" begin
    # Not a table at all
    @test_throws ArgumentError interactions_to_sparse(rand(3, 3))

    # Missing columns
    @test_throws ArgumentError interactions_to_sparse((a=[1], item=[2]))
    @test_throws ArgumentError interactions_to_sparse((user=[1], a=[2]))
    @test_throws ArgumentError interactions_to_sparse(
        (user=[1], item=[2]); value_col=:missing_col)

    # Empty table
    @test_throws ArgumentError interactions_to_sparse((user=Int[], item=Int[]))
    ...
    @test_throws DimensionMismatch interactions_to_sparse(
